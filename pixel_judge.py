...
class TrafficlightDetector:
    ...
    def _valid_boxes_judge(self, boxes):
        ...
        for box in boxes:
            ...
            # 縦横比の確認
            if 1.8 <= (y2-y1)/(x2-x1) <= 2.2:
                upper_hsv = hsv[y1:y1+h//2, x1:x1+w]
                lower_hsv = hsv[y1+h//2:y1+h, x1:x1+w]
                ...
                if np.sum(upper_red) > 0 and np.sum(lower_blue) > 0:
                    ...
                    valid_boxes.append(box)
                cv2.rectangle(self._input_cvimg, (x1,y1), (x2,y2), color, thickness)
        return valid_boxes
    ...
    def _yolo(self, input_img):
        ...
        max_conf = -1
        max_conf_class = None
        yolo_output = self._model(input_img, classes=[15, 16], conf=self._min_conf) #self._model() returns list of class:ultralytics.engine.results.Results
        ...
        if(max_conf_class is not None):
            if((max_conf_class==16 and max_conf > self._conf_threshold_blue) or
               (max_conf_class==15 and max_conf > self._conf_threshold_red)):
                return yolo_output[0], yolo_output[0].names.get(max_conf_class)
            else:
                rospy.logwarn("UNDER THRESHOLD")
                return yolo_output[0], 'under_threshold'
        else:
    ...
    def _judge_signal(self) -> str:
        ...
        yolo_output, signal = self._yolo(input_img)

        if(signal == 'no_boxes'):
            self._visualize()
            rospy.logerr("NOT WORKING")
            return 'not_working'
        elif(signal == 'under_threshold'):
            valid_boxes = self._valid_boxes_judge(yolo_output.boxes)
            # signal = self.pixel_judge(valid_boxes)
            img = self._draw_box(input_img, valid_boxes)
            self._visualize(img)
            # self._visualize(yolo_output.plot())
            return signal
        else:
